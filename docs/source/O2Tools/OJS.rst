...
:Returns: An ``OjsHeader`` object that has all the information from the header.
:Return Type: ``OjsHeader``
...
:Returns: A ``list`` that has all the frames in the ``OjsFrame`` format.
:Return Type: ``list``
...
* ``file_path (str)``: The file path of the new OJS file.
* ``header (OjsHeader)``: The header of the new OJS file.
* ``frames (list)``: A list of all the frames in the new OJS file.

----

:Returns: The OJS file in the form of the ``OJS`` class.
:Return Type: ``OJS``